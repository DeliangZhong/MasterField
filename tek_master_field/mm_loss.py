...
ansatz) for every loop C in a precomputed LoopSystem (from cluster/
...
MM equation (candidate D from `cluster/mm_equations.py`):
...
CAVEAT (physics): candidate D is the LEADING-ORDER strong-coupling form; it
omits the contact term of the exact lattice loop equation
(cuntz_bootstrap/lattice_loop_eq.py). This loss is therefore a soft/approximate
constraint, not a rigorous QCD2 gate.

Reusable parts from cluster/lattice.py: LoopSystem, build_loop_system,
signed_area_2d.
"""
...
# lattice.py lives in cluster/, NOT master_field/ (which has no such module —
# the old path here raised ModuleNotFoundError at import time).
