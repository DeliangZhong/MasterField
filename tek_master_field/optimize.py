...
def coupling_continuation(
    ...
    WARNING (physics): ``tek_loss`` is λ-INDEPENDENT in this functional form
    (see ``optimize_tek`` / ``tek.tek_loss``: λ enters only via logging). Every
    stage therefore minimises the *identical* objective — the schedule changes
    only the warm-start init and the per-λ labels, and all stages converge to the
    same saddle up to the basin reached from ``p_init``. This routine is a
    diagnostic of optimiser robustness / basin structure across warm-starts, NOT
    a trace of coupling-dependent physics. Do not read λ-dependence into the
    returned dict. A genuinely λ-dependent objective requires adding the coupling
    into the action (e.g. an MM/loop-equation residual weighted by λ).
    """
    warnings.warn(
        "coupling_continuation runs a λ-schedule over a λ-independent TEK "
        "objective: every stage optimises the same loss (λ affects only "
        "warm-start/logging). Results carry no coupling dependence.",
        stacklevel=2,
    )
