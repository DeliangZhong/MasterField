...
def main():
    ...
    parser.add_argument("--coupling", type=float, default=0.5, help="Coupling constant g")
    parser.add_argument("--max_word_length", type=int, default=12, help="Loop truncation level L")
    parser.add_argument("--n_epochs", type=int, default=5000, help="Number of optimisation epochs")
    parser.add_argument("--lr", type=float, default=1e-2, help="Learning rate")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument(
        "--validate", action="store_true", help="Run validation against exact solutions"
    )
    parser.add_argument("--bootstrap", action="store_true", help="Run SDP bootstrap for bounds")
    parser.add_argument(
    ...
    if args.model in ["gaussian", "quartic", "sextic"]:
        ...
        # Save results
        np.save(
            os.path.join(args.output_dir, f"moments_{args.model}_g{args.coupling}.npy"),
            sol["moments"],
        )
        np.save(
        ...
        np.save(
            os.path.join(args.output_dir, f"free_cumulants_{args.model}_g{args.coupling}.npy"),
            sol["free_cumulants"],
        )
        ...
        if args.validate or args.model == "gaussian":
            ...
            if m_exact is not None:
                ...
                if args.model in ["gaussian", "quartic"]:
                    ...
                    fock_test = CuntzFockSpace(
                        n_matrices=1, max_length=min(8, args.max_word_length)
                    )
                    M_test = fock_test.build_master_field_voiculescu(
                        v_coeffs[: min(len(v_coeffs), 7)]
                    )
                    m_fock = fock_test.compute_moments(
                        M_test, max_power=min(8, args.max_word_length)
                    )
