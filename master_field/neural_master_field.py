...
class CholeskyMasterField:
    ...
    def __init__(self, n_matrices: int, max_word_length: int,
        ...
        from schwinger_dyson import all_cyclic_words
        self.basis_words = [()]
...
def init_r_transform_params(key: jax.Array, n_coeffs: int, 
                            hidden_dim: int = 128, n_layers: int = 3) -> Dict:
...
def r_transform_predict(params: Dict, z: jnp.ndarray, 
                        use_network: bool = False,
                        coupling: float = 0.0) -> jnp.ndarray:
...
def moments_from_r_transform(kappa: jnp.ndarray, max_moment: int) -> jnp.ndarray:
    ...
    for n in range(1, max_moment + 1):
        ...
        val = sum(float(kappa[p - 1]) * S[n - 1, p - 1] 
                  for p in range(1, min(n, n_kappa) + 1))
        m = m.at[n].set(val)
...
def init_cuntz_operator_params(key: jax.Array, fock_dim: int, 
                                n_matrices: int, max_coeffs: int = 10) -> Dict:
    """Initialise master field operator M̂ = Σ c_k x̂^k in Fock space.
    
    For Hermitian representation: M̂ = λ(x̂) where λ is a polynomial.
    """
    keys = random.split(key, n_matrices)
    params = {}
...
def sd_loss_two_matrix(moment_func: Callable, g: float, 
                       test_words: List[Tuple], 
                       interaction: str = "commutator_squared") -> jnp.ndarray:
    ...
    for w in test_words:
        for a in range(2):
            ...
            if interaction == "commutator_squared":
                ...
                word2 = (b, b, a) + w
                word3 = (b, a, b) + w
                lhs += (g2 / 2) * (moment_func(word1) + moment_func(word2)
                                    - 2 * moment_func(word3))
...
class MasterFieldTrainer:
    ...
    def __init__(self, model_name: str, n_matrices: int = 1,
                 coupling: float = 0.0, max_word_length: int = 10,
                 lr: float = 1e-3, n_epochs: int = 5000):
    ...
    def init(self, key: jax.Array):
        ...
        if self.n == 1:
            ...
            self.cholesky_model = CholeskyMasterField(
                self.n, self.L, hidden_dim=256, n_layers=4)
...
class MultiMatrixTrainer:
    ...
    def __init__(self, n_matrices: int = 2, coupling: float = 1.0,
                 max_word_length: int = 6, interaction: str = "commutator_squared",
                 lr: float = 1e-3, n_epochs: int = 10000):
        self.n = n_matrices
        self.g = coupling
        self.L = max_word_length
    ...
    def init(self, key: jax.Array):
        ...
        schedule = optax.warmup_cosine_decay_schedule(
            init_value=self.lr * 0.01, peak_value=self.lr,
            warmup_steps=500, decay_steps=self.n_epochs)
        self.optimizer = optax.chain(
            optax.clip_by_global_norm(1.0),
            optax.adam(schedule))
        self.opt_state = self.optimizer.init(self.params)
    ...
    def loss_fn(self, params: Dict) -> jnp.ndarray:
        ...
        moment_func = lambda w: self.moment_from_matrix(Omega, w)
        
        loss = sd_loss_two_matrix(moment_func, self.g, self.test_words, 
                                  self.interaction)
...
if __name__ == "__main__":
    ...
    print("\n--- Gaussian model ---")
    trainer = MasterFieldTrainer("gaussian", n_matrices=1, coupling=0.0,
                                 max_word_length=12, lr=1e-2, n_epochs=3000)
    ...
    print("\n--- Quartic model (g=0.5) ---")
    trainer_q = MasterFieldTrainer("quartic", n_matrices=1, coupling=0.5,
                                    max_word_length=12, lr=1e-2, n_epochs=5000)
    ...
    for k in range(0, 11, 2):
        print(f"  m_{k}: ML = {sol_q['moments'][k]:.8f}, exact = {m_exact[k]:.8f}, "
